...
def train(model, epochs, optimizer, 
          criterion, train_loader, train_data, 
          val_loader, val_data, device, writer):
...
def test(model, test_loader, device):
    ...
    for batch_data in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            for c in range(k):
                value = torch.eq(outputs[:,c].argmax(dim=-1), labels[:,c].argmax(dim=-1))
...
def main():
    ...
    seq2cond = {"sag-T1" : ["left_neural_foraminal_narrowing", 
                ...
                "ax-T2" : ["left_subarticular_stenosis", 
    ...
    if epochs > 0 :
        exceptions, epoch_loss_values, metric_values = train(model, epochs, optimizer, 
          criterion, train_loader, train_data, 
          val_loader, val_data, device, writer)
