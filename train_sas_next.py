...
import pandas as pd
import monai
from convnext import ConvNeXtAxial3D, ConvNeXt3D, ConvNeXt2D
...
def get_transforms(mode='basic', side='left'):
    ...
    to_25d_transform = Compose([
        # Permuter les axes pour faire de la profondeur Z une dimension de canal
        # Originalement: [C, X, Y, Z] -> Résultat souhaité: [C*Z, X, Y]
        Lambdad(keys=['image'], func=lambda x: x.permute(0, 3, 1, 2).reshape(-1, x.shape[1], x.shape[2]))
    ])
    ...
    second_transforms_random = Compose([
        ...
        SpatialPadd(keys=['image'], spatial_size=(60, 80, 5)),  # Padding pour atteindre une taille fixe
        RandSpatialCropd(keys=['image'], roi_size=(60, 80, 5), random_size=False),  # Crop pour obtenir une taille fixe
    ...
    if mode == 'basic':
        if side == 'left':
            common_transforms = Compose([first_transforms, second_transforms_basic, to_25d_transform, final_transforms])
        elif side == 'right':
            common_transforms = Compose([first_transforms, right_flip, second_transforms_basic, to_25d_transform, final_transforms])

    elif mode == 'random':
        if side == 'left':
            common_transforms = Compose([first_transforms, second_transforms_random, to_25d_transform, final_transforms])
        elif side == 'right':
            common_transforms = Compose([first_transforms, right_flip, second_transforms_random, to_25d_transform, final_transforms])
...
def train_and_evaluate_model(device, data_dir, csv_file, batch_size=4, lr=5e-5, epochs=40, val_split=0.25, augment=False):
    ...
    model = ConvNeXt2D(in_chans=5, num_classes=3)
    ...
    model_name = f"sas_next_model_epochs_{epochs}_lr_{lr}_augmentation_{augment}"
